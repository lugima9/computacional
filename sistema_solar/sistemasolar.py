...
def calcAcelPlaneta(estePlaneta, pos, masas, numeroPlanetas):
...
# Función que calcula un paso del algoritmo de Verlet para obtener la posición 
# y velocidad de cada planeta en el tiempo.
# Recibe: paso, masas, posiciones, velocidades, núm. de planetas
# Devuelve: array de posiciones, array de velocidades
def pasoVerlet(h, m, r, v, numeroPlanetas):

    #Creo array de acel. 3D de (nº planetas) x 2 (x,y)
    acel = np.zeros((numeroPlanetas, 2))

    # Para cada uno de los planetas...
    for i in range(numeroPlanetas):
        #Calcula la aceleración (x,y) del planeta       
        acel[i] = calcAcelPlaneta(i, r, m, numeroPlanetas)
...
# Recibe: nombre del fichero con una fila por planeta, bool de alineación
# Devuelve: nombres, masas, posiciones, velocidades (reescalados)
...
# Devuelve: array de masas, array de posiciones y array de velocidades(reescalado)
...
# Recibe: array de masas, array de posiciones y array de velocidades
# Devuelve: array de masas, array de posiciones y array de velocidades (desescalado)
...
# Devuelve: array de posiciones y array de velocidades con formato (x,y) para cada planeta
...
# Recibe: instante, arrays de posición, array de velocidad de cada planeta, paso, array bool de comprobación, núm. planetas
# Devuelve: array de periodos, array bool de comprobación
def calcPeriodoAlin(t, r, v, paso, obtenido, numeroPlanetas):
...
# Recibe: array de periodos calculados, fichero con periodos tabulados, núm. planetas
# Devuelve: array con periodos tabulados, array de error relativo por planeta
def errorRelPeriodo(periodos, ficheroTab, numeroPlanetas):
...
# Recibe: array de masas, array posiciones, array de velocidades, núm. de planetas
# Devuelve: módulo del momento angular total del Sistema Solar
def calcMomAng(m, r, v, numeroPlanetas):
...
# Recibe: array de masas, array de posiciones, array de velocidades, núm. planetas
# Devuelve: energía total del Sistema Solar
def calcEnergia(m, r, v, numeroPlanetas):
...
if eleccion == 2 and alinear == True:
    ...
    while t < t_max:
        ...
        r, v = pasoVerlet(h, masas, r, v, numeroPlanetas)

        #HACER ESTO CON FUNCIONES APARTE, QUE SI NO ES UN PUTO LÍO

        #--CÁLCULO DEL PERIODO--
        periodos, obtenido= calcPeriodoAlin(t, r, v, h, obtenido, numeroPlanetas)
        
        #--CÁLCULO DEL MOMENTO ANGULAR--
        momAngMod = calcMomAng(masas, r, v, numeroPlanetas)
        momAngArr = np.append(momAngArr, momAngMod)

        #--CÁLCULO DE LA ENERGÍA--
        energia = calcEnergia(masas, r, v, numeroPlanetas)
    ...
    periodosTab, errorRel = errorRelPeriodo(periodos, "periodos_tab.dat", numeroPlanetas)
